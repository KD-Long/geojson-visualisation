...
var require_earcut = __commonJS({
  "node_modules/earcut/src/earcut.js"(exports, module) {
    ...
    function isEar(ear) {
      ...
      var p = c.next;
      while (p !== a) {
        if (p.x >= x02 && p.x <= x12 && p.y >= y0 && p.y <= y1 && pointInTriangle(ax, ay, bx, by, cx, cy, p.x, p.y) && area(p.prev, p, p.next) >= 0)
          return false;
        p = p.next;
      }
      return true;
    ...
    function isEarHashed(ear, minX, minY, invSize) {
      ...
      while (p && p.z >= minZ && n && n.z <= maxZ) {
        if (p.x >= x02 && p.x <= x12 && p.y >= y0 && p.y <= y1 && p !== a && p !== c && pointInTriangle(ax, ay, bx, by, cx, cy, p.x, p.y) && area(p.prev, p, p.next) >= 0)
          return false;
        p = p.prevZ;
        if (n.x >= x02 && n.x <= x12 && n.y >= y0 && n.y <= y1 && n !== a && n !== c && pointInTriangle(ax, ay, bx, by, cx, cy, n.x, n.y) && area(n.prev, n, n.next) >= 0)
          return false;
        n = n.nextZ;
      }
      while (p && p.z >= minZ) {
...
function interpolate_default(a, b) {
  var x02 = a[0] * radians, y0 = a[1] * radians, x12 = b[0] * radians, y1 = b[1] * radians, cy0 = cos(y0), sy0 = sin(y0), cy1 = cos(y1), sy1 = sin(y1), kx0 = cy0 * cos(x02), ky0 = cy0 * sin(x02), kx1 = cy1 * cos(x12), ky1 = cy1 * sin(x12), d = 2 * asin(sqrt(haversin(y1 - y0) + cy0 * cy1 * haversin(x12 - x02))), k = sin(d);
